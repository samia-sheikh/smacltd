...
const SingleCategoryCourses = () => {
  ...
  const { courseParentCategoryId } = useParams();
  ...
  const getParentCategory = async () => {
    await fetchData(
      ...
      `/api/course/parent/${courseParentCategoryId}`,
      undefined,
      (res) => {
  ...
  const getCoursesOfSingleProductCategory = async () => {
    await fetchData(
      //here instead of parent category, privide id to get
      `/api/course?parentCategory=${courseParentCategoryId}`,
      undefined,
      (res) => {
        //console.log(res, "resssss");
        setCourses(res.data);
  ...
  const getTopCourseSubCategories = async () => {
    await fetchData(
      //here instead of parent category, privide id to get
      `/api/course/sub?parentCategoryId=${courseParentCategoryId}`,
      undefined,
      (res) => {
        //console.log(res, "resssss");
  ...
  const getCourses = async (
    courseSubCategoryId = undefined,
    courseName = undefined
  ) => {
    await fetchData(
      //here instead of sub category, privide id to get
      `/api/course/getBySubCat?${
        courseSubCategoryId && courseName
          ? `courseSubCategoryId=${courseSubCategoryId}&courseName=${courseName}`
          : !courseSubCategoryId && courseName
  ...
  const filterCourses = async () => {
    ...
    if (element) {
      ...
      getCourses(element.courseSubCategoryId, searchValue);
    } else if (selectedSubCategory?.courseSubCategoryId) {
      // console.log("else if", selectedSubCategory, searchValue);
      getCourses(selectedSubCategory?.courseSubCategoryId, searchValue);
    } else if (
      searchValue &&
      !element?.courseSubCategoryId &&
      !selectedSubCategory?.courseSubCategoryId
    ) {
