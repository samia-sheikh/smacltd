...
const ProductSubCategories = ({ setProductCategory }) => {
  ...
  const { singleParentCategoryProduct } = useSelector(
    (state) => state.parentCategories
  );
  useEffect(() => {
    fetchData(
      `/api/product/sub?parentCategoryId=${singleParentCategoryProduct?.productParentCategoryId}`,
      undefined,
      (res) => {
  ...
  const handleGlobalSearch = (event) => {
    ...
    try {
      fetchData(
        `/api/admin/product/sub/search/${singleParentCategoryProduct?.productParentCategoryId}/?searchQuery=${event.target.value}`,
        undefined,
        (res) => {
          if (res) {
            setProductCategories(res.data);
  ...
  return (
    <>
      ...
      <AddSubCategory
        ...
        url={"/api//admin/product/sub"}
        parentCategory={singleParentCategoryProduct}
      ...
      {deleteCategory ? (
        <DeleteCategory
          ...
          url={`api/admin/product/sub/${deleteCategory?.productSubCategoryId}`}
          anyChanges={setAnyCategoryChanges}
        />
      ) : null}
      <Box
        ...
        >
          ...
          >
            ...
            <Typography component={"span"} variant="h4">
              {singleParentCategoryProduct?.name}
            </Typography>
          </Box>
          <Box sx={{ width: "170px" }}>
            <ButtonComp
