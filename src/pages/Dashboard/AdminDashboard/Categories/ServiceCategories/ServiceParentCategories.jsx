...
import { setSingleParentCategoryService } from "../../../../../features/slice/categoriesSlice";
...
const ServiceParentCategories = ({ setServiceCategory }) => {
  ...
  // here i'm fetching the Parent Categories just Paret Categories not all Categories
  useEffect(() => {
    fetchData("/api/admin/service/parent/all", undefined, (res) => {
  ...
  const handleGlobalSearch = (event) => {
    ...
    try {
      fetchData(
        `/api/admin/service/parent/search?searchQuery=${event.target.value}`,
        undefined,
        (res) => {
          if (res) {
            //   setServiceCategories(res.data);
    ...
    <>
      ...
      {categoryToEdit && (
        <EditParentCategory
          ...
          url={`/api/admin/service/parent/${categoryToEdit?.serviceParentCategoryId}`}
          anyChanges={setAnyCategoryChanges}
        />
      )}
      {/* {deleteCategory ? (
        <DeleteCategory
          ...
          url={`/api/admin/service/parent/${deleteCategory?.serviceParentCategoryId}`}
          anyChanges={setAnyCategoryChanges}
        />
      ) : null} */}
      <Box
                ...
                <TableCell width={230}>
                  ...
                  <Box sx={{ display: "flex", gap: "10px" }}>
                    <ButtonComp
                      ...
                      click={(prev) => {
                        dispatch(
                          setSingleParentCategoryService({ data: category })
                        );
...
export default ServiceParentCategories;
